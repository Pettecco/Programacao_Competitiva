...
double BS(int vetor[], double inicio, double fim, int area, int numTiras)
{
  ...
  if (abs(soma - (double)area) < 0.0001)
    return meio;
  else if (soma > (double)area)
    return BS(vetor, meio, fim, area, numTiras);
  else if (soma < (double)area)
    return BS(vetor, inicio, meio, area, numTiras);
  return meio;
}
...
int main()
{
  ...
  while ((N > 0) and (A > 0))
  {
    ...
    for (int i = 0; i < N; i++)
    {
      cin >> tiras[i];
      total += tiras[i];
      if (bigger < (double)tiras[i])
        bigger = (double)tiras[i];
    }
    if (total < A)
      cout << "-.-" << endl;
    else if (total == A)
